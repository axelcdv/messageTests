...
#define kSpringLength   0
#define kSpringFrequency    0.7
...
- (void)prepareLayout {
    ...
    CGFloat padding = 100;
    CGRect currentRect = CGRectMake(0,
                                    contentOffset.y - padding,
                                    self.collectionView.frame.size.width,
                                    self.collectionView.frame.size.height + 2 * padding);
    ...
    NSArray *newVisibleItems = [itemsInCurrentRect
                                filteredArrayUsingPredicate:
                                [NSPredicate
                                 predicateWithBlock:^BOOL(UICollectionViewLayoutAttributes *item, NSDictionary *bindings) {
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (layoutAttributes == nil) {
        ...
        UIAttachmentBehavior *spring = [[UIAttachmentBehavior alloc] initWithItem:layoutAttributes
                                                                 attachedToAnchor:[layoutAttributes center]];
        spring.length = kSpringLength;
        spring.damping = kSpringDamping;
        spring.frequency = kSpringFrequency;
